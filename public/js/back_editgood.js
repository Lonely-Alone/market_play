...
var iFrameCallBack = function(uploadResult) {
	...
	if(isLegal === "false"){
		$testRemind($("uploadTrigger"+target), "图片大小请控制在5M以内", 1);
...
addGoodBtn.addEvent('click', function(){
	var mainPicDiv = $("mainPicDiv"), previewPicDiv = $("previewPicDiv"), nameInput = $("nameInput"), chiefInput = $("chiefInput"), showPriceInput = $("showPriceInput"), 
		realPriceInput = $("realPriceInput"), leftCountInput = $("leftCountInput"), descrArea = $("descrArea"),goodType=$("goodType"),goodPlace=$("goodPlace");
	var picUrls = '';
	$$('div.commPic img').each(function(item){
		if(item.attr('origin').trim().length>0){
	...
	if($isEmpty(nameInput)){
		...
		$testRemind(goodType, "请选择商品类型", 1);
		setTimeout('$testRemind.hide()',3000);
	}else if($isEmpty(goodPlace)){
		$testRemind(goodPlace, "请选择商品归属", 1);
		setTimeout('$testRemind.hide()',3000);
		...
		$testRemind(mainPicDiv, "请输入商品描述", 1);
		setTimeout('$testRemind.hide()',3000);
		...
		setTimeout('$testRemind.hide()',3000);
	}else if(picUrls.split(",").length<4){
		$testRemind(mainPicDiv, '您必须上传&nbsp;<span class="co">2</span>&nbsp;张商品附图', 1);
		setTimeout('$testRemind.hide()',3000);
	}else{
		new AjaxPost(addGoodBtn, {
			...
			data: {
				...
				limitCount:limitCountInput.val().trim(),
				descr: ue.getContent(),
				...
				goodTypeId:goodType.val().trim(),
				goodPlace:goodPlace.val().trim()
			},
